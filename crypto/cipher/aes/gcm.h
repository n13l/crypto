...
int
aes_gcm_encrypt(u8 *output, const unsigned char *input, int input_length,
...
/*
 * The same two with associated data: |aad_len| bytes at |aad| are
 * authenticated but not encrypted, which is what an AEAD protocol's header
 * needs (TLS authenticates a record's sequence number, type, version and
 * length this way). |aad| may be NULL when |aad_len| is 0, and then these are
