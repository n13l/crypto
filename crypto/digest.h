...
enum algorithm_digest {
	...
	/*
	 * Appended, not inserted: the values 1..9 above are what a suite row,
	 * a counter table and a key log all already say, and MD5 is the one
	 * digest that arrived after them. It is here for the TLS 1.0/1.1 key
	 * schedule (modules/digest/md5/md5.c) and is off in every default
...
#ifndef __CRYPTO_DIGEST_MD5_H__
...
static inline void
md5_update(struct md5 *ctx, const u8 *data, unsigned int len)
{
	arch_md5_update(ctx, data, len);
}

static inline void
md5_final(struct md5 *ctx, u8 *out)
{
	arch_md5_final(ctx, out);
}
