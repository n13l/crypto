...
({ \
	...
	SAFE_MOVE_PDU(lv, avail, 2);\
	for (unsigned __i = 0; __i < len; __i++) { \
		u16 num16 = be16_cpu(*((be16*)lv)); \
		block; \
		SAFE_MOVE_PDU(lv, avail, sizeof(be16));  \
	}\
});
...
{ \
	...
	SAFE_MOVE_PDU(payload, avail, sizeof(*val)); \
	block \
}
...
#define SAFE_READ_BE16(lv, avail, val)         \
	CHECK_AVAIL(2, avail, -1);           \
	val = be16_cpu(*((be16*)lv)); \
	SAFE_MOVE_PDU(lv, avail, 2) 

#define SAFE_READ_BE24(lv, avail, val)         \
	CHECK_AVAIL(3, avail, -1);           \
	val = be24_cpu(lv); \
	SAFE_MOVE_PDU(lv, avail, 3) 

#define SAFE_READ_BE32(lv, avail, val)         \
	CHECK_AVAIL(4, avail, -1);           \
	val = be32_cpu(*((u32*)(lv))); \
	SAFE_MOVE_PDU(lv, avail, 4) 

#define SAFE_READ_BE16_REQUIRE(lv, avail, val, expected) \
	SAFE_READ_BE16(lv, avail, val); \
	REQUIRE_U16(val, expected)

#define SAFE_READ_LV_BUF_U8(lv, avail, ptr, size, maxsize) \
...
#define SAFE_READ_LV_BUF_BE16(lv, avail, ptr, size, maxsize) \
...
{ \
	...
	if (size_not_used > 0) { \
		SAFE_MOVE_PDU(lv, avail, size_not_used); \
	} \
} \
