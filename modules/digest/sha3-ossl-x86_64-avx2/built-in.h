...
/*
 * The sponge, and the permutation it calls, which goes in the same object as
 * the sponge. The declarations are the family's - every SHA-3 backend has these
 * seven entry points - and <modules/digest/sha3-arch/decl.h> says which build
 * compiles the bodies where, and why.
 *
 * The bodies are this backend's own, and are the reason it does not include
 * <modules/digest/sha3-arch/arch.h> like the other four: OpenSSL's AVX2 code
 * absorbs whole rate blocks itself, so SHA3_absorb() takes the buffer and the
 * rate and returns what is left over, and SHA3_squeeze() reads the digest out.
 * The shared sponge xors and permutes a block at a time around a permutation
 * that does neither, which is a different shape, not a slower one.
 */
