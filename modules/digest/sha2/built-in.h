...
/*
 * Public context types embedded by callers (and by HMAC/PRF). The generic
 * backend reinterprets these as its internal sha256_ctx/sha512_ctx working
 * state, so they must mirror that layout exactly. This definition is why the
 * backend defines __MODULES_DIGEST_SHA2_H__ above: it fully supersedes the
 * fallback struct in <modules/digest/sha2.h>.
 */
struct sha256 {
	u32          h[8];
	unsigned int len;
