...
void aes128_set_iv(struct aes128_ctx* ctx, const u8* iv)
{
	memcpy (ctx->Iv, iv, AES_BLOCKLEN);
}
...
{
  for (int i = 0; i < aes128_BLOCKLEN; ++i) // The block in AES is always 128bit no matter the key size
  {
    buf[i] ^= Iv[i];
  }
}

void aes128_cbc_encrypt(struct aes128_ctx *ctx, u8* buf, u32 length)
{
  uintptr_t i;
  u8 *Iv = ctx->Iv;
  for (i = 0; i < length; i += aes128_BLOCKLEN)
  {
    XorWithIv(buf, Iv);
...
void aes128_cbc_decrypt(struct aes128_ctx* ctx, u8* buf,  u32 length)
{
  uintptr_t i;
  u8 storeNextIv[aes128_BLOCKLEN];
  for (i = 0; i < length; i += aes128_BLOCKLEN)
  {
    memcpy(storeNextIv, buf, aes128_BLOCKLEN);
    InvCipher((state_t*)buf, ctx->RoundKey);
    XorWithIv(buf, ctx->Iv);
    memcpy(ctx->Iv, storeNextIv, aes128_BLOCKLEN);
    buf += aes128_BLOCKLEN;
  }
...
static void aes128_ctr_xcrypt(struct aes128_ctx* ctx, u8* buf, u32 length)
{
  u8 buffer[aes128_BLOCKLEN];

  unsigned i;
  int bi;
  for (i = 0, bi = aes128_BLOCKLEN; i < length; ++i, ++bi)
  {
    if (bi == aes128_BLOCKLEN) /* we need to regen xor compliment in buffer */
    {

      memcpy(buffer, ctx->Iv, aes128_BLOCKLEN);
