...
#ifdef AES_DECRYPTION          // We ONLY need reverse for decryption
...
void aes_init_keygen_tables( void )
{
    ...
    FSb[0x00] = 0x63;
#ifdef AES_DECRYPTION  // whether AES decryption is supported
    RSb[0x63] = 0x00;
#endif /* AES_DECRYPTION */
...
/*
 * ...and the call, because every entry point below is unusable until it has
 * happened: aes_setkey() refuses while the tables are unbuilt, and the round
 * key pointer it would have set stays null.
 *
 * The registration glue next door (module.c) already calls
 * aes_init_keygen_tables() from a constructor, but module.c is only linked
 * into the loadable-module build - a built-in CONFIG_CRYPTO_CIPHER_AES_GENERIC
 * gets cbc128.o, cbc256.o and this file and nothing else. So the constructor
 * belongs here, next to the tables it builds, where it runs in both. It is
 * idempotent (aes_init_keygen_tables() returns early once the flag is set),
 * which is what makes having it in both places harmless.
 */
static void __init__ aes_gcm_tables_init( void )
{
    aes_init_keygen_tables();
}
                ...
                uchar *output )         // pointer to destination data
{
    ...
    /*
     * Whether the running GHASH is carried at all. Here it is interleaved
     * with the counter rather than a pass of its own, so what a decrypt that
     * nothing will authenticate saves is the accumulate and the multiply
     * inside this loop. Folded to a constant in every build that does verify,
     * which is the default - see CONFIG_CRYPTO_VERIFIED_DECRYPT_AEAD.
     */
#ifdef CONFIG_CRYPTO_VERIFIED_DECRYPT_AEAD
    const int auth = 1;
#else
    const int auth = ( ctx->mode == ENCRYPT );
#endif
...
#ifndef CONFIG_CRYPTO_VERIFIED_DECRYPT_AEAD
    /* No GHASH was carried, so check_tag holds the mask and nothing else;
       there is nothing to compare it against. See
       CONFIG_CRYPTO_VERIFIED_DECRYPT_AEAD. */
    (void)tag; (void)diff; (void)i;
    return( 0 );
#else
...
/*
 * AEAD front-ends: encrypt appends a 16-byte tag after the ciphertext;
 * decrypt expects that tag trailing the ciphertext and verifies it. See the
 * contract in <crypto/cipher/aes/gcm.h>. No associated data is authenticated.
 */
#define AES_GCM_TAG_LEN 16

int aes_gcm_encrypt(u8 *output, const u8 *input, int input_length,
         const u8* key, const size_t key_len, const u8 *iv, const size_t iv_len)
{
    ...
    if( gcm_setkey( &ctx, key, (const uint)key_len ) != 0 )
        return( GCM_AUTH_FAILURE );  // an unusable key is not a crash

    ret = gcm_crypt_and_tag( &ctx, ENCRYPT, iv, iv_len, NULL, 0,
                            input, output, input_length,
                            output + input_length, AES_GCM_TAG_LEN );
         ...
         const u8* key, const size_t key_len, const u8 *iv, const size_t iv_len)
{
    ...
    if ( ct_len < 0 )           // must carry at least a full tag
        return( GCM_AUTH_FAILURE );

    if( gcm_setkey( &ctx, key, (const uint)key_len ) != 0 )
        return( GCM_AUTH_FAILURE );  // an unusable key is not a crash

    ret = gcm_auth_decrypt( &ctx, iv, iv_len, NULL, 0,
                            input, output, ct_len,
                            input + ct_len, AES_GCM_TAG_LEN );
...
/*
 * The associated-data forms. See the contract in <crypto/cipher/aes/gcm.h>;
 * here they are the same one-shots with the |add| argument gcm_auth_decrypt()
 * and gcm_crypt_and_tag() have always taken, passed through instead of NULL.
 */
int
         ...
         const u8 *key, const size_t key_len, const u8 *iv, const size_t iv_len)
{
    ...
    ret = gcm_crypt_and_tag( &ctx, ENCRYPT, iv, iv_len, aad, aad_len,
                             input, output, (size_t)input_length,
                             output + input_length, AES_GCM_TAG_LEN );
