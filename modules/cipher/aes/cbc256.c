...
void aes256_cbc_set_iv(struct aes256_ctx* ctx, const u8* iv)
{
	memcpy (ctx->Iv, iv, AES_BLOCKLEN);
}
...
void aes256_cbc_encrypt(struct aes256_ctx *ctx, u8* buf, u32 length)
{
  uintptr_t i;
  u8 *Iv = ctx->Iv;
...
void aes256_cbc_decrypt(struct aes256_ctx* ctx, u8* buf,  u32 length)
{
  uintptr_t i;
...
static void aes128_ctr_xcrypt(struct aes256_ctx* ctx, u8* buf, u32 length)
{
