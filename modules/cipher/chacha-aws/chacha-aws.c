...
static int have_neon(void)
{
	...
	return 1;	/* AArch64 always has Advanced SIMD */
#else
	/* ARMV7_NEON == bit 0; only ever populated under runtime accel
	 * (modules/cpu/arm-ossl/cap.c), so a build-time config that never calls
	 * crypto_init() would otherwise be pinned to the scalar path. */
	return OPENSSL_armcap_P & 1u;
#endif
}
	       ...
	       const uint32_t key[8], const uint32_t counter[4])
{
	if (len == 0)
		return;
	/* The neon path wants >= 192B. */
	if (len >= 192 && have_neon())
