...
static u32
crc32c_intel_le_hw_byte(u32 crc, u8 const *data, unsigned long length)
{
	while (length--) {
 ...
 */
static u32
crc32c_intel(u8 const *data, unsigned long length)
{
	...
	if (iremainder)
		crc = crc32c_intel_le_hw_byte(crc, (u8 *)ptmp,
				 iremainder);
