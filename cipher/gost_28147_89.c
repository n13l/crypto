...
static void
gost_encrypt(u32 const in[2], u32 out[2], u32 const key[8])
{
...
static void
gost_decrypt(u32 const in[2], u32 out[2], u32 const key[8])
{
	register u32 n1, n2; /* As named in the GOST */
...
static void
gost_ofb(u32 const *in, u32 *out, int len, u32 const iv[2], u32 const key[8]) {
	u32 temp[2];         /* Counter */
	u32 gamma[2];        /* Output XOR value */

	/* Compute starting value for counter */
	gost_encrypt(iv, temp, key);
...
static void
gost_cfb_encrypt(u32 const *in, u32 *out, int len, u32 iv[2], u32 const key[8])
{
	while (len--) {
		gost_encrypt(iv, iv, key);
		iv[0] = *out++ ^= iv[0];
		iv[1] = *out++ ^= iv[1];
	}
}
...
static void
gost_cfb_decrypt(u32 const *in, u32 *out, int len, u32 iv[2], u32 const key[8])
{
	u32 t;
	while (len--) {
		gost_encrypt(iv, iv, key);
...
static void
gost_mac(u32 const *in, int len, u32 out[2], u32 const key[8])
{
	register u32 n1 = 0, n2 = 0; /* As named in the GOST */
	while (len--) {
		n1 ^= *in++;
		n2 = *in++;
...
#define RAND32 ((u32)rand() << 17 ^ (u32)rand() << 9 ^ rand())
...
main(void) {
	...
	for (i = 0; i < 1000; i++) {
		...
		gost_encrypt(plain, cipher, key);
		for (j = 0; j < 99; j++)
			gost_encrypt(cipher, cipher, key);
		for (j = 0; j < 100; j++)
			gost_decrypt(cipher, cipher, key);
